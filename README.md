...
> ### 🔑 First run prints your settings password — SAVE IT
>
> The launcher **auto-generates** a settings password on first run and prints it
> **exactly once** in a `settings: AUTO-GENERATED …` block. It unlocks editing API
> keys and env vars right in the GUI (Settings → **SERVER ENV**) on this machine.
> It is never stored in plaintext and never printed again.
>
> - **Human installing?** Copy it somewhere safe before you scroll past.
> - **Agent installing?** Capture it and **deliver it to your owner** through a
>   direct channel — never commit it, never post it to logs, rooms, or issues.
> - Prefer your own? `--password <value>`. Rotate anytime:
>   `node skill/agent-face/scripts/settings-password.mjs`.
...
Set any of them in the GUI (Settings → SERVER ENV — unlocked by the auto-generated
password the launcher printed on first run) or in
[`.env.example`](.env.example)-style `.env.local`.
</details>
...
| `FACE_SETTINGS_PASSWORD_HASH` | The GUI env editor (Settings → SERVER ENV) — **auto-generated by the launcher on first run, plaintext printed once**; rotate via `settings-password.mjs` |
