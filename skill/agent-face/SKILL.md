...
| `FACE_SETTINGS_PASSWORD_HASH` | Unlocks the GUI env editor (Settings → SERVER ENV) — auto-generated by `start.mjs` on first run (plaintext printed once; rotate via `settings-password.mjs`). Containers don't run the launcher: provision with `settings-password.mjs` or the localhost GUI bootstrap |
